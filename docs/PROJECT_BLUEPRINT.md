...
本藍圖涵蓋四部分：**系統運作方式**、**客戶使用時的前後端路線**、**DevOps 路線**、**全端技術路線**。所有路徑與檔案以 repo 為準（[docs/NOTION_SYNC.md](NOTION_SYNC.md)、[proxy.ts](../proxy.ts)、[app/api/webhook/line](../app/api/webhook/line)）。
...
單一客戶從註冊到在 Dashboard 回覆一條 LINE 訊息的逐步流程見 [docs/CUSTOMER_JOURNEY.md](CUSTOMER_JOURNEY.md)。
...
- **流程**：push main → Vercel 自動 build + deploy；正式網址 https://www.customeraipro.com。
- **環境變數**：Vercel 專案 Settings → Environment Variables（與 [docs/DEPLOYMENT_AND_ENV_FAQ.md](DEPLOYMENT_AND_ENV_FAQ.md) 對齊）；multi-bot production 必填 Upstash Redis 與 `LINE_BOT_ENCRYPTION_KEY`。
...
- **程式碼品質**：CodeQL、dependency-review 在 CI（若已啟用）；branch protection 建議依 [docs/BRANCH_PROTECTION.md](BRANCH_PROTECTION.md) 設定。
...
- 依 [.cursor/rules/engineering-status.mdc](../.cursor/rules/engineering-status.mdc) 處理 P0/P1 技術債。
...
- 依 [docs/DEEP_DIVE_IMPROVEMENTS.md](DEEP_DIVE_IMPROVEMENTS.md) 與 engineering-status.mdc 的 P2/P3 排程。

---

與 [NOTION_SYNC.md](NOTION_SYNC.md)、[PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md)、[API_ENDPOINTS.md](API_ENDPOINTS.md) 並列，供 onboarding 與維運對照。
